...
def get_report_data(glue_conn_name, tmp_dir_path):
    ...
    df_detail_summed = df_detail.groupBy("PART_SSN","ACTIVITY","POST_DATE").agg(round(sum("EMPLOYEE"),2).alias("EMPLOYEE"),round(sum("AUTOMATIC"),2).alias("AUTOMATIC"),round(sum("MATCHING"),2).alias("MATCHING"),round(sum("ROW_TOTAL"),2).alias("ROW_TOTAL")).withColumn("FUND",lit("ZZ").cast(StringType())).withColumn("AE",lit(".")).withColumn("RV",lit(".")).fillna(" ").fillna(0)
    ...
    list_data_detail = sorted(list(map(lambda row: row.asDict(), df_detail1.collect())),key=lambda x: str(x['POST_DATE'])+"~"+str(x["ACTIVITY"])+"~"+str(x["FUND"]))
